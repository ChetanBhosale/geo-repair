...
export function MessageContent({ className, ...props }: MessageContentProps) {
  return (
    <div
      className={cn(
        "grid w-full gap-2 overflow-hidden rounded-lg bg-primary p-4 text-sm",
        className
      )}
      {...props}
    />
...
export const MessageResponse = memo(
  ({ className, ...props }: MessageResponseProps) => (
    <Streamdown
      className={cn(
        "prose prose-sm dark:prose-invert max-w-none text-secondary [&_code]:rounded [&_code]:bg-secondary [&_code]:px-1 [&_code]:py-0.5 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0",
        className
      )}
