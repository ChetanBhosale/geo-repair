...
export function AuditReport({ report }: { report: SiteReport }) {
  const findings = [...report.findings].sort((a, b) => {
    const rank = { fail: 0, mixed: 1, partial: 2, "not-applicable": 3, pass: 4 } as const;
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <ScoreStat label="GEO" value={report.pillars.geo.score} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{findings.length} checks across the site</CardDescription>
        </CardHeader>
            ...
            <div key={f.id} className="flex items-start justify-between gap-4 py-3">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  ...
                  {f.affectedCount > 0 ? ` · ${f.affectedCount} affected page(s)` : ""}
                  {f.representativeEvidence ? ` · ${f.representativeEvidence}` : ""}
                </p>
              </div>
              <Badge variant={statusVariant(f.siteStatus)}>{f.siteStatus}</Badge>
            </div>
          ))}
        </CardContent>
      </Card>
...
function ScoreStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex flex-col">
      ...
      <span className={`text-2xl font-semibold ${scoreColor(value)}`}>{Math.round(value)}</span>
    </div>
  );
}
