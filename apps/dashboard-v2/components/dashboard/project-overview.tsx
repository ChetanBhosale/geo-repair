...
export function ProjectOverview({ projectId }: { projectId: string }) {
  ...
  useBreadcrumbs([{ label: project.data?.name ?? "Project" }])
  ...
  const projectPath = project.data ? projectOverviewPath(project.data) : null
  const agentPath = React.useCallback(
    (run: Pick<AgentRunSummary, "slug">) =>
      project.data ? projectAgentRunPath(project.data, run) : null,
    [project.data]
  )
  ...
  const clearCheckoutReturnParams = React.useCallback(() => {
    ...
    router.replace(
      routeWithQuery(projectPath ?? `/dashboard/projects/${projectId}`, nextQuery)
    )
  }, [projectId, projectPath, router, searchParams])
  ...
  async function onConfirmPaidStart() {
    if (openAgentRun) {
      const path = agentPath(openAgentRun)
    ...
    try {
      ...
      if (project.data) {
        router.push(
          projectAgentRunPath(project.data, { slug: created.agentRunSlug })
        )
      }
    } catch (err) {
  ...
  async function startAgentFromOrder(orderId: string) {
    ...
    if (project.data) {
      router.push(projectAgentRunPath(project.data, { slug: created.agentRunSlug }))
    }
  }
