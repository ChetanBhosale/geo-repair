...
export function AgentRunScreen({
  ...
  return (
    <AgentProvider agentRunId={agentRunId}>
      <AgentScreen projectId={projectId} />
    </AgentProvider>
  )
}

function AgentScreen({ projectId }: { projectId: string }) {
  ...
  useBreadcrumbs([
    {
      ...
      href: project.data ? projectOverviewPath(project.data) : undefined,
    },
    { label: "Agent run" },
  ])
          ...
          <div className="flex h-full w-full flex-col pt-4">
            ...
            >
              <div className="mx-auto w-full max-w-3xl space-y-3 py-5 pr-8 pl-6">
                ...
                {failed ? (
                  <FailedRunMessage run={run} projectId={projectId} />
                ) : null}
              </div>
            </div>
...
function FailedRunActions({
  ...
  const project = useProject(projectId)
  const projectHref = project.data
    ? projectOverviewPath(project.data)
