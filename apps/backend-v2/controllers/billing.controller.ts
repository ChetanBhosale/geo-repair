...
import {
  ...
  reconcilePublicFixCheckoutReturn,
} from "../functions/billing.service";
import type { DodoWebhookHeaders } from "../functions/providers/dodo";
...
export async function createFixCheckout(req: Request, res: Response) {
  try {
    ...
    const projectId =
      stringBodyValue(body.projectId) ?? stringBodyValue(body.repositoryId);
    if (!projectId) {
    ...
    const result = await createFixCheckoutForProject({
      userId: req.userId!,
      projectId,
...
export async function getPublicOrderStatus(req: Request, res: Response) {
  try {
    const orderId = stringBodyValue(req.params.id);
    if (!orderId) return res.status(400).json({ error: "id is required." });
...
export async function reconcileCheckoutReturn(req: Request, res: Response) {
  try {
    ...
    const result = await reconcileFixCheckoutReturn({
      ...parsed.data,
      userId: req.userId!,
    });
...
export async function reconcilePublicCheckoutReturn(
  req: Request,
  res: Response,
) {
  try {
    ...
    const result = await reconcilePublicFixCheckoutReturn(parsed.data);
    return res.json(result);
  } catch (err) {
    return sendBillingError(res, err);
  }
