...
Rules:
- mode AUTO = safe structural/markup fix over existing content (metadata, JSON-LD, robots, sitemap, llms.txt, canonical, alt text, semantic HTML, charset, doctype, viewport) OR a hand-written AEO-delivery fix (markdown twin route/handler, content-negotiation middleware serving the twin to AI clients, the X-Robots-Tag/Vary/X-Markdown-Tokens/Link headers, the nosniff/X-AEO-Version conformance headers, and .md twin URLs in the sitemap). No user input. For AEO delivery, never add a third-party dependency to the user's repo: write framework-idiomatic code.
