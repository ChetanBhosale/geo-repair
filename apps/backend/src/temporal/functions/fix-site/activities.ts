...
/**
 * Build the agent's fix plan for a site: run a fresh authoritative full checkup, then group the
 * rubric findings into site-wide vs per-page tasks.
...
export async function planSiteFix(input: FixSiteInput): Promise<FixPlan> {
  ...
  return buildFixPlan(report.findings, { pagesChecked: report.crawl.pagesChecked });
}

// Activities for the fix-site queue.
//
// The planning half (checkup -> grouped task list) is implemented. The execution half: spin up an
