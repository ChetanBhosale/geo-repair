...
pass = all four present. partial = some present. fail = none.
...
- HTML response: append a `Link: </path.md>; rel="alternate"; type="text/markdown"` header. In
  Next.js this is the middleware/route response headers (the HTML `<link>` tag alone does NOT pass).
- Optional niceties: `X-Content-Type-Options: nosniff`, `X-AEO-Version: 1.0`.
...
the HTML shows the `Link` alternate header. Build passes.
